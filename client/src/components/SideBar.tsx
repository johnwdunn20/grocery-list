...
const SideBar = ({sideBarOpen, closeSideBar}) => {
  ...
  return (
    <>
      <nav id="sideMenu" className={`fixed top-0 left-0 w-64 lg:w-96 h-full z-20 bg-secondaryBackgroud border-2 shadow-sm transform ${sideBarOpen ? '-translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out`}>
        <div className='flex justify-between content-center border-b-2 border-slate-500'>
          ...
          <button className='rounded-sm' onClick={closeSideBar}>
